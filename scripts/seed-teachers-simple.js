...
async function seedTeachers() {
    ...
    for (let i = 0; i < teachersData.length; i++) {
        ...
        try {
            ...
            const userExists = usersList.users.length > 0;
            ...
            if (!userExists) {
                try {
                    // Create user with basic info only (no phone number)
                    ...
                    // Try to add phone number separately (optional)
                    try {
                        const cleanPhone = teacher.phone.replace(/-/g, '');
                        const phoneNumber = cleanPhone.startsWith('+') ? cleanPhone : `+998${cleanPhone}`;
                        console.log(`   📞 Converting phone: ${teacher.phone} → ${phoneNumber}`);
                        
                        await users.updatePhone(userId, phoneNumber);
                        console.log(`   📞 Added phone number: ${phoneNumber}`);
                    } catch (phoneError) {
                        console.log(`   ⚠️  Could not add phone number: ${phoneError.message}`);
                    }
                } catch (error) {
