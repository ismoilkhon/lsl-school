...
import { useStudents } from "@/lib/hooks/useQueries";

const CountChartContainer = () => {
  const { getUserRole } = useAuthStore();
  const { data: students = [], isLoading, error } = useStudents();
  
  // Only show data for admin users
  ...
  // Calculate boys and girls count from real data
  ...
  if (error) {
    return (
      <div className="bg-white rounded-xl w-full h-full p-4">
        ...
        <div className="flex items-center justify-center h-32">
          <p className="text-gray-500 text-sm">Failed to load student data: {error.message}</p>
        </div>
      </div>
    );
  }
